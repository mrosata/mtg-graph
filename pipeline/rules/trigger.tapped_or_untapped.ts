...
export const rule: Rule = {
  ...
  match: (t) => {
    ...
    // v0.50 (S24) — plural forms admitted (Rewrite History: "whenever one
    // or more creatures you control become tapped").
    const passive = /when(?:ever)? (?:[^.]*?) (?:becomes? tapped|becomes? untapped|is tapped|is untapped|are tapped|are untapped|enters tapped|enters untapped)/;
