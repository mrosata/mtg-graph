...
import { rule } from './effect.has_earthbend';
...
describe('effect.has_earthbend', () => {
  it.each([[['Earthbend']], [['Earthbend', 'Flying']]])('matches: %j', (kw) => {
    expect(rule.matchCard!(card(kw), '')).toBeTruthy();
  });

  it.each([[[]], [['Waterbend']], [['Firebending']], [['Airbend']]])('does not match other bending keywords: %j', (kw) => {
