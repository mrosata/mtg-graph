...
const PATTERNS = [
  ...
  /\b(?:has|have)\s+no\s+cards?\s+in\s+(?:your|their|a player's|each opponent's|an opponent's)?\s*hand\b/,
  // (2) At-most-1: "one or fewer cards in <hand>"
  /\bone\s+or\s+fewer\s+cards?\s+in\s+(?:your|their|a player's|each opponent's|an opponent's)?\s*hand\b/,
  // (3) Per-card scaling: "for each card in <hand>"
  /\bfor\s+each\s+card\s+in\s+(?:your|their|a player's|each opponent's|an opponent's)\s+hand\b/,
  // (4) Number-of scaling: "(the )?number of cards in <hand>"
  /\b(?:the\s+)?number\s+of\s+cards\s+in\s+(?:your|their|a player's|each opponent's|an opponent's)\s+hand\b/,
  // (5) Threshold count: "<N> or more cards in <hand>"
  /\b(?:\d+|two|three|four|five|six|seven|eight|nine|ten)\s+or\s+more\s+cards?\s+in\s+(?:your|their|a player's|each opponent's|an opponent's)?\s*hand\b/,
];
