...
# Batch resolved 2026-06-01 — v0.29.0 (19 of 21 fix groups shipped, 2 rejected)

## Rejected (intentional prior behavior — do not re-flag):

- **Rise of the Dark Realms** — proposed broadening `condition.cares_graveyard` to admit "cards from … graveyards". REJECTED: explicit negative test row at `condition.cares_graveyard.test.ts:70` (reanimate effects belong on `effect.reanimate`, not cares_graveyard).
- **Shivan Dragon** — proposed narrowing `effect.grants_stat_buff` to exclude "this creature gets +N/+N". REJECTED: self-pump intentionally admitted per user memory `project_v021_non_evasion_grants.md` and existing positive test row.

---

## Shipped (rule changes landed; see commit message and v0.29.0 diff):

# Audit Batch A — 2026-06-01

---

## Rise of the Dark Realms  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.cast_noncreature_spell`, `effect.is_instant_or_sorcery`, `effect.reanimate`

### Issues

- **missing**: `condition.cares_graveyard`
  - **What's wrong:** Card scales off the entire graveyard pool (all creature cards from all graveyards) — quintessential graveyard-cares effect, but rule only matches "cards in [a] graveyard" not "from [a] graveyard".
  - **Evidence vs reality:** oracle says "all creature cards from all graveyards"; current PATTERNS look for `cards? in ... graveyards?` and miss the `from ... graveyards` phrasing.
  - **Suggested fix:** broaden `condition.cares_graveyard` PATTERNS to accept `cards? (?:in|from) ... graveyards?` for the unconditional pull-all-creatures phrasing.

---

## River's Rebuke  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
- **missing**: `effect.bounce_creature` / `effect.bounce_artifact` / `effect.bounce_enchantment` / `effect.bounce_planeswalker` / `effect.bounce_or_blink`
  - **What's wrong:** Single-sided mass bounce ("return all nonland permanents target player controls to their owner's hand") is the canonical River's Rebuke effect — should fire every typed bounce tag plus the umbrella.
  - **Evidence vs reality:** oracle says "return all nonland permanents ... to their owner's hand"; no bounce rule matches this "all nonland permanents target player controls" mass-bounce phrasing.
  - **Suggested fix:** extend the bounce family (and/or `effect.bounce_or_blink`) to admit "return all nonland permanents [target player controls / any player controls] to their owner's hand" so it propagates to all typed bounce children.

---

## Shivan Dragon  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.grants_stat_buff`, `effect.has_activated_ability`, `effect.has_flying`, `effect.has_mana_activated_ability`

### Issues

- **false-positive**: `effect.grants_stat_buff`
  - **What's wrong:** Self-only pump activated ability ("this creature gets +1/+0") is being tagged as an anthem-style grant. The tagDef explicitly calls this "anthem-style" — buffing a group / other creatures — but Shivan Dragon's ability only pumps itself.
  - **Evidence vs reality:** evidence was `"creature gets +1/+0"`, but the leading "this" makes the subject self-referential — there's no anthem (no "creatures you control", no "target creature", no other-creature reference).
  - **Suggested fix:** narrow PATTERN so the bare `creatures?` alternative excludes the literal "this creature" frame — either require a non-self qualifier (`creatures? (?:you control|each player controls)` etc.) before `get(s)`, or add a negative lookbehind on `\bthis ` for the creatures? branch.

---

## Sire of Seven Deaths  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.has_first_strike`, `effect.has_lifelink`, `effect.has_menace`, `effect.has_reach`, `effect.has_trample`, `effect.has_vigilance`, `effect.has_ward`, `effect.life_changed`

### Issues

- **false-positive**: `effect.life_changed`
  - **What's wrong:** Ward—Pay 7 life is a cost an opponent pays to target this creature; it's not an effect Sire of Seven Deaths produces. Tagging it as `effect.life_changed` leaks the card into "drain / lifegain matters" graphs.
  - **Evidence vs reality:** evidence was `"pay 7 life"`, but the surrounding context is the Ward keyword's em-dash cost frame ("Ward—Pay N life"), not a self-cast activation cost or generic life-payment effect.
  - **Suggested fix:** narrow the PAY regex in `effect.life_changed` to exclude ward-cost frames — e.g. add a negative lookbehind `(?<!ward—)` (or strip the ward suffix before scanning, similar to how reminder text is stripped).

---

## Skyknight Squire  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_tribe.knight`, `effect.counter_modified`, `effect.gains_keyword_self_conditional`, `effect.plus_one_counter`, `trigger.another_creature_etb`

### Issues

- **false-positive**: `condition.cares_tribe.knight`
  - **What's wrong:** Card transforms ITSELF into a Knight under a counter condition; doesn't reference Knights as a payoff axis (no "Knights you control get…" or "for each Knight"). The existing strip in `condition.cares_tribe` handles the "becomes a Knight in addition to" frame but misses the "is a Knight in addition to its other types" frame.
  - **Evidence vs reality:** evidence was `"knight"`, matched in "is a Knight in addition to its other types" — that's the same self-typing transformation pattern the v0.22.0 Possessed Goat fix already targeted, just with the "is" verb instead of "becomes".
  - **Suggested fix:** extend `becomesTribePattern` in `condition.cares_tribe.ts` to also strip `\\bis\\s+(?:a|an)\\s+...\\b<tribe>\\b in addition to (its other|all other) (colors|types|colors and types)` — same anchor tail, just admit "is" as an alternative to "becomes".

---

## Sphinx of Forgotten Lore  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_graveyard`, `condition.cares_instant_sorcery_in_graveyard`, `effect.has_flash`, `effect.has_flying`, `trigger.attack_or_block`

### Issues

- **missing**: `effect.grants_cast_from_graveyard`
  - **What's wrong:** The combat trigger grants flashback to an instant/sorcery in your graveyard — that's a license to cast a card from the graveyard, the exact semantics this tag exists for, but via the flashback-keyword frame rather than the bare "you may cast" frame.
  - **Evidence vs reality:** oracle says "target instant or sorcery card in your graveyard gains flashback until end of turn"; the rule's POSITIVE_PATTERNS only match "you may cast/play <X> from your graveyard" verbatim and miss the gains-flashback-grant frame.
  - **Suggested fix:** add a POSITIVE_PATTERN for `\\bgains? (?:flashback|jump-start|disturb|escape|unearth) (?:until end of turn|this turn)?\\b` when the target/subject is `card in (?:your|a|an opponent's) graveyard` — grants_keyword-induced graveyard-cast license.

---

## Steel Hellkite  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.board_wipe`, `effect.destroy_artifact`, `effect.destroy_creature`, `effect.destroy_enchantment`, `effect.destroy_permanent`, `effect.destroy_planeswalker`, `effect.grants_stat_buff`, `effect.has_activated_ability`, `effect.has_flying`, `effect.has_mana_activated_ability`

### Issues

- **false-positive**: `effect.board_wipe`
  - **What's wrong:** Steel Hellkite is a single-target, single-player, combat-damage-gated kill, not a symmetric board sweep.
  - **Evidence vs reality:** evidence was `"destroy each nonland permanent"`, but the destruction is restricted to permanents of mana value X whose controller was dealt combat damage by this creature this turn — typically one player, often zero permanents. The tagDef says "Destroys or exiles all creatures (or all of a category) at once," which doesn't fit a conditional pingscope effect.
  - **Suggested fix:** narrow `effect.board_wipe` to exclude "destroy each ... whose controller was dealt combat damage" / "whose controller has lost life" gated frames (or require non-conditional sweep frames).

---

## Suspicious Shambler  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
- **false-positive**: `effect.has_activated_ability`
  - **What's wrong:** The only activated ability is a graveyard-zone activation (analogous to embalm/unearth), not a battlefield activation. The tagDef explicitly scopes to "This permanent (creature, artifact, enchantment, land, or planeswalker) has at least one activated ability" — i.e., on the battlefield, where Training Grounds-style payoffs and tap interactions live.
  - **Evidence vs reality:** evidence was `"{4}{b}{b}, exile this card from your graveyard:"`, but the cost line itself ("Exile this card from your graveyard") signals the ability functions only in the graveyard.
  - **Suggested fix:** exclude activated-ability matches whose cost includes "exile this/it from your graveyard" / "exile ~ from your graveyard" (and similarly for hand-activated abilities like Mayhem/Madness). Same fix applies to `effect.has_mana_activated_ability`.
- **false-positive**: `effect.has_mana_activated_ability`
  - **What's wrong:** Same as above — the activated ability is a graveyard ability, not an on-battlefield mana-activated ability that would be reducible by Training Grounds.
  - **Evidence vs reality:** evidence was `"{4}{b}{b}, exile this card from your graveyard:"`, but the cost requires exiling from graveyard; the card has no battlefield-zone activated abilities.
  - **Suggested fix:** exclude graveyard-cost activations from `has_mana_activated_ability` (same regex carve-out as above).

---

## Thousand-Year Storm  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_noncreature_spell`, `effect.copy_spell`, `trigger.spell_cast`

### Issues

- **missing**: `condition.cares_spells_cast_this_turn`
  - **What's wrong:** Thousand-Year Storm scales its copy count "for each other instant and sorcery spell you've cast before it this turn" — the canonical storm-style spells-cast-this-turn count. The tagDef explicitly calls out "storm-grant payoff axis."
  - **Evidence vs reality:** oracle has `"for each other instant and sorcery spell you've cast before it this turn"`, which is exactly the spells-cast-this-turn ordinal/count axis.
  - **Suggested fix:** broaden `condition.cares_spells_cast_this_turn` to match `for each ... spell you('?ve)? cast ... this turn` (covering the "before it this turn" / "earlier this turn" variants).

---

## Tinybones, Bauble Burglar  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.draws_or_discards`, `effect.has_activated_ability`, `effect.has_mana_activated_ability`, `effect.targeted_discard`, `trigger.card_drawn_discarded`

### Issues

- **missing**: `effect.exile_from_graveyard`
  - **What's wrong:** The trigger "exile it from their graveyard with a stash counter on it" moves a discarded card (which goes to graveyard) into exile — a true exile-from-graveyard effect.
  - **Evidence vs reality:** oracle has `"exile it from their graveyard with a stash counter on it"`, which is the exact tagDef pattern ("removes cards from a graveyard by exiling them").
  - **Suggested fix:** add this anaphoric "exile it from their graveyard" form to the `effect.exile_from_graveyard` regex anchors.
- **missing**: `effect.cast_from_exile`
  - **What's wrong:** "During your turn, you may play cards you don't own with stash counters on them from exile" grants permission to cast/play cards directly from the exile zone — exactly the tagDef's "play from exile" frame.
  - **Evidence vs reality:** oracle has `"you may play cards you don't own with stash counters on them from exile"`, matching the "play ... from exile" pattern.
  - **Suggested fix:** broaden `effect.cast_from_exile` to match `play cards ... from exile` permission frames (current matcher may scope to "cast" only).
- **missing**: `effect.counter_modified`
  - **What's wrong:** "exile it ... with a stash counter on it" places a (non-+1/+1) counter on a card in exile. The tagDef is generic counter placement.
  - **Evidence vs reality:** oracle has `"with a stash counter on it"`. Stash counters are a real counter type and placement counts as counter modification.
  - **Suggested fix:** broaden `effect.counter_modified` to recognize "with a <named> counter on it" frames for non-+1/+1 counters (stash, etc.), or accept this as a known omission for exotic counter names.
- **missing**: `condition.cares_exile_pile`
  - **What's wrong:** Tinybones operationalizes the exile zone as a play resource — playing cards from exile with stash counters is the textbook exile-pile-matters pattern.
  - **Evidence vs reality:** oracle has `"play cards you don't own with stash counters on them from exile"`, matching "cards exiled with this permanent" / exile-as-resource use.
  - **Suggested fix:** broaden `condition.cares_exile_pile` to include "play cards ... from exile" license frames on the same card that exiles the cards.
---

## Valkyrie's Call  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_tribe.angel`, `effect.counter_modified`, `effect.grants_evasion`, `effect.plus_one_counter`

### Issues

- **missing**: `trigger.creature_dies`
  - **What's wrong:** Card has a "whenever a … creature you control dies" trigger but the dies-trigger tag isn't applied.
  - **Evidence vs reality:** oracle text says `"Whenever a nontoken, non-Angel creature you control dies"` — textbook creature-dies trigger.
  - **Suggested fix:** broaden `trigger.creature_dies` regex to match `whenever a [qualifiers] creature you control dies` patterns with intervening qualifier words.

- **missing**: `effect.reanimate`
  - **What's wrong:** Card returns dying creatures from graveyard to the battlefield (textbook reanimation) but `effect.reanimate` not applied.
  - **Evidence vs reality:** "return that card to the battlefield under its owner's control" after the creature dies — that's reanimation.
  - **Suggested fix:** broaden `effect.reanimate` to recognize `return that card to the battlefield` patterns when the antecedent is a creature that just died.
---

## Aatchik, Emerald Radian  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_artifacts`, `condition.cares_graveyard`, `effect.counter_modified`, `effect.create_creature_token`, `effect.create_token`, `effect.life_changed`, `effect.plus_one_counter`, `trigger.self_etb`

### Issues

- **missing**: `trigger.creature_dies`
  - **What's wrong:** Card has a "whenever another Insect you control dies" trigger but the dies-trigger tag isn't applied.
  - **Evidence vs reality:** oracle text says `"Whenever another Insect you control dies"` — typed-tribe variant of the standard another-creature-dies frame.
  - **Suggested fix:** broaden `trigger.creature_dies` to match `whenever another <tribe> you control dies` (tribe-qualified another-creature-dies frame).
---

## Adrenaline Jockey  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.counter_modified`, `effect.deals_damage`, `effect.plus_one_counter`, `trigger.spell_cast`

### Issues

- **missing**: `condition.cares_activated_abilities`
  - **What's wrong:** Card triggers on activating an exhaust ability — an activated-ability payoff axis.
  - **Evidence vs reality:** oracle text says `"Whenever you activate an exhaust ability"` — the description for `cares_activated_abilities` mentions "activate triggers" explicitly.
  - **Suggested fix:** broaden `condition.cares_activated_abilities` to match `whenever you activate an? .* ability` patterns (with optional exhaust/loyalty/etc qualifiers).
---

## Afterburner Expert  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.counter_modified`, `effect.has_activated_ability`, `effect.has_exhaust`, `effect.has_mana_activated_ability`, `effect.plus_one_counter`, `effect.reanimate`

### Issues

- **missing**: `condition.cares_activated_abilities`
  - **What's wrong:** Card triggers on activating an exhaust ability — an activated-ability payoff axis.
  - **Evidence vs reality:** oracle text says `"Whenever you activate an exhaust ability"` — `cares_activated_abilities` description includes "activate triggers".
  - **Suggested fix:** broaden `condition.cares_activated_abilities` to match `whenever you activate an? .* ability` (same pattern as Adrenaline Jockey).
---

## Ancient Vendetta  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
- **missing**: `effect.exile_from_graveyard`
  - **What's wrong:** Card explicitly searches an opponent's graveyard and exiles cards from it — textbook graveyard-hate.
  - **Evidence vs reality:** oracle text says `"Search target opponent's graveyard, hand, and library for up to four cards with that name and exile them"`.
  - **Suggested fix:** broaden `effect.exile_from_graveyard` regex to match `search target opponent's graveyard.* and exile` patterns (Surgical Extraction / Extirpate / Cling to Dust family).

- **missing**: `effect.exile_from_library`
  - **What's wrong:** Card searches an opponent's library for matching cards and exiles them.
  - **Evidence vs reality:** oracle text says `"Search target opponent's … library for up to four cards with that name and exile them"`.
  - **Suggested fix:** broaden `effect.exile_from_library` to match `search target … library … and exile` patterns.
---

## Bleachbone Verge  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_lands`, `effect.add_mana`, `effect.has_activated_ability`

### Issues

- **false-positive**: `condition.cares_lands`
  - **What's wrong:** The "you control a Plains or a Swamp" gate is a land-type activation requirement for a dual-land cycle (Verge cycle); it's not the "lands matter" payoff axis (landfall / lands-as-a-resource / land-count scaling).
  - **Evidence vs reality:** evidence was `"you control a plains"`, but this only enforces that the second mana ability requires a basic land type, not a scaling / payoff on lands.
  - **Suggested fix:** narrow `condition.cares_lands` to exclude "Activate only if you control a <BasicLandType>" gates (Verge cycle); these are dual-land activation requirements, not lands-matter payoffs.

---

## Brightfield Mustang  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.counter_modified`, `effect.has_saddle`, `effect.plus_one_counter`, `trigger.attack_or_block`

### Issues

- **missing**: `effect.untap`
  - **What's wrong:** The attack-while-saddled trigger says "untap it and put a +1/+1 counter on it" — a self-untap.
  - **Evidence vs reality:** oracle text has `"untap it"` as part of the trigger, which matches the tagDef ("Untaps a target permanent").
  - **Suggested fix:** broaden `effect.untap` to catch anaphoric "untap it" when "it" refers to the card itself (post-attack saddled trigger pattern).

---

## Brightglass Gearhulk  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_low_mana_value`, `effect.has_first_strike`, `effect.has_trample`, `trigger.self_etb`

### Issues

- **missing**: `effect.tutors_artifact`, `effect.tutors_creature`
  - **What's wrong:** ETB tutors for up to two artifact/creature/enchantment cards from library. No tutor tag fired.
  - **Evidence vs reality:** oracle text has `"search your library for up to two artifact, creature, and/or enchantment cards"` — this is a clear tutor that lands artifact and creature cards into hand.
  - **Suggested fix:** broaden `effect.tutors_artifact` and `effect.tutors_creature` regexes to handle the "and/or" multi-type tutor frame (e.g. "artifact, creature, and/or enchantment cards"). Consider adding `effect.tutors_enchantment` if it doesn't exist.

---

## Caelorna, Coral Tyrant  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
- **data-ingest gap**: card has empty `oracleText` and `normalizedText` in the artifact, so no rules can fire.
  - **What's wrong:** Caelorna, Coral Tyrant is a printed card with abilities (a Ward and/or graveyard interaction depending on the printing), but the artifact stripped it down to an empty string.
  - **Evidence vs reality:** evidence was `(none)`; the artifact lacks the oracle entirely, which suggests a Scryfall ingestion edge case (possibly multi-face / DFC handling, or this set/printing has unusual oracle structure).
  - **Suggested fix:** check `pipeline/stripScryfallCard` (or face concatenation logic) for the printing of this card — likely a face-array vs top-level oracle text edge case for this set.

---

## Caradora, Heart of Alacria  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_subtype.mount`, `condition.cares_subtype.vehicle`, `effect.tutors_subtype.mount`, `effect.tutors_subtype.vehicle`, `trigger.self_etb`

### Issues

- **missing**: `effect.plus_one_counter`, `condition.cares_plus_one_counter`
  - **What's wrong:** Caradora is a +1/+1-counter doubler replacement effect — "that many plus one +1/+1 counters are put on it instead". The doubling sub-case is in the description of `effect.plus_one_counter`, and the replacement-effect frame is exactly the cares-plus-one-counter axis.
  - **Evidence vs reality:** oracle text has `"if one or more +1/+1 counters would be put on ... that many plus one +1/+1 counters are put on it instead"` — neither tag fired.
  - **Suggested fix:** broaden `effect.plus_one_counter` and `condition.cares_plus_one_counter` regexes to cover the "if one or more +1/+1 counters would be put on … that many plus one +1/+1 counters are put on it instead" replacement-effect doubler frame (Hardened Scales / Branching Evolution / Caradora pattern).

---

## Chandra, Spark Hunter  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_subtype.vehicle`, `effect.create_creature_token`, `effect.create_token`, `effect.draws_or_discards`, `effect.grants_haste`, `effect.sacrifice_artifact`, `trigger.beginning_of_combat`, `trigger.another_artifact_etb`

### Issues

- **false-positive**: `effect.create_creature_token`
  - **What's wrong:** "Create a 3/2 colorless Vehicle artifact token with crew 1" produces a non-creature artifact token (Vehicle), not a creature token. Vehicles only become creatures when crewed.
  - **Evidence vs reality:** evidence was `"create a 3/2 colorless vehicle artifact token"`, but a Vehicle artifact token is NOT a creature token — the rule's "tighter than `effect.create_token`, excludes Treasures/Roles/Foods" wording implies Vehicles should similarly be excluded.
  - **Suggested fix:** narrow `effect.create_creature_token` to exclude tokens whose primary type is Vehicle / Artifact (without "creature" in the type line). Token must be "<color> <type> creature token" where "creature" is present, or token must list a creature subtype without competing non-creature subtype.

---

## Coalstoke Gearhulk  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_low_mana_value`, `effect.counter_modified`, `effect.grants_deathtouch`, `effect.grants_evasion`, `effect.grants_haste`, `effect.has_deathtouch`, `effect.has_menace`, `effect.reanimate`, `trigger.beginning_of_end_step`, `trigger.self_etb`

### Issues

- **missing**: `effect.exile_creature` (or `effect.exile_from_battlefield`)
  - **What's wrong:** The end-step trigger says "exile that creature" — exiles a battlefield creature. No exile-from-battlefield tag fired.
  - **Evidence vs reality:** oracle text has `"exile that creature"` at end step, which is an exile clause on a creature already in play.
  - **Suggested fix:** broaden `effect.exile_creature` to match the anaphoric "exile that creature" form used by reanimator-with-exile-clause cards (Coalstoke / Soul Salvage / many sticker rentals).

---

## Crash and Burn  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_subtype.vehicle`, `effect.cast_noncreature_spell`, `effect.deals_damage`, `effect.is_instant_or_sorcery`

### Issues

- **missing**: `effect.destroy_artifact` (or general destroy tag)
  - **What's wrong:** "Destroy target Vehicle" — Vehicles are artifacts. The card should match `effect.destroy_artifact`, since `destroy_artifact` includes broad/multi-type destroy phrasings that cover artifacts.
  - **Evidence vs reality:** oracle text has `"destroy target vehicle"`. No destroy tag fired despite Vehicle being a strict artifact subtype.
  - **Suggested fix:** broaden `effect.destroy_artifact` to include the "destroy target Vehicle" frame (Vehicles are artifacts by rule). Same fix likely applies to `effect.exile_artifact` for "exile target Vehicle".

---

## Cursecloth Wrappings  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_graveyard`, `condition.cares_tribe.zombie`, `effect.grants_stat_buff`, `effect.has_activated_ability`

### Issues

- **missing**: `effect.grants_cast_from_graveyard`
  - **What's wrong:** The activated ability grants embalm to a graveyard creature card — embalm is a "cast from graveyard" keyword, so this is licensing a graveyard cast. Description of `grants_cast_from_graveyard` covers Muldrotha-style licenses.
  - **Evidence vs reality:** oracle text has `"target creature card in your graveyard gains embalm until end of turn"` — granting embalm IS granting a cast-from-graveyard ability.
  - **Suggested fix:** broaden `effect.grants_cast_from_graveyard` to include "gains <gy-keyword> until end of turn" frames (Cursecloth-style grants of embalm/flashback/jump-start to graveyard cards).

---

## Daretti, Rocketeer Engineer  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `condition.cares_artifacts`, `condition.cares_graveyard`, `effect.sacrifice_artifact`, `trigger.attack_or_block`, `trigger.self_etb`

### Issues

- **missing**: `effect.reanimate`
  - **What's wrong:** "Choose target artifact card in your graveyard … return the chosen card to the battlefield" is a textbook reanimation. The two-step "choose then return" phrasing isn't matching the reanimate rule.
  - **Evidence vs reality:** oracle text has `"choose target artifact card in your graveyard"` … `"return the chosen card to the battlefield"`. The reanimate rule appears to require "return target <X> card from your graveyard to the battlefield" inline.
  - **Suggested fix:** broaden `effect.reanimate` to match the "choose target <type> card in your graveyard … return the chosen card to the battlefield" split-phrasing frame (Daretti style).

---

## Demonic Junker  <!-- audited 2026-06-01, ruleVersion v0.28.0 -->
...
**Current tags:** `effect.counter_modified`, `effect.destroy_creature`, `effect.has_activated_ability`, `effect.plus_one_counter`, `trigger.self_etb`

### Issues

- **missing**: `condition.cares_artifacts`, `effect.cost_reduction`
  - **What's wrong:** "Affinity for artifacts" is a cost-reducing keyword that scales with artifacts you control. Neither the cares-artifacts axis nor cost-reduction fired.
  - **Evidence vs reality:** normalized text retains `"affinity for artifacts"` (reminder stripped). Affinity by definition cares about artifact count.
  - **Suggested fix:** add `"affinity for artifacts"` as a positive frame for `condition.cares_artifacts` (and similarly for any other affinity-type X subtype), and have it imply `effect.cost_reduction`. Alternatively add a `effect.has_affinity` tag akin to other keyword `has_*` tags.


# Batch 2 resolved 2026-06-01 — v0.30.0 (25 of 30 fix groups shipped, 4 rejected, 1 partial, 1 skipped, 1 deferred)
...
- **The Last Ride** — `effect.life_changed` exclude activated "Pay N life" cost REJECTED (v0.30.0). v0.15 intentional design — Bonecache Overseer "{T}, pay 1 life: draw a card" is a positive at `effect.life_changed.test.ts:51`. Controller-paid life IS life loss.
- **Earthrumbler, Mimeoplasm, Molt Tender, Winter Cursed Rider** — `effect.exile_from_graveyard` admit non-self cost-form REJECTED (v0.30.0). Test rows at `effect.exile_from_graveyard.test.ts:67-68` encode cost-form negatives; the rule architecture deliberately separates cost vs effect axis.
- **Push the Limit** — `trigger.beginning_of_end_step` exclude "next end step" REJECTED (v0.30.0). Anzrag's Rampage (also a Sorcery with the same delayed-trigger shape) is encoded as positive at `trigger.beginning_of_end_step.test.ts:20-21`. Excluding would regress.

## Partial / skipped / deferred:

- **Group 15 partial** — `effect.tutors_creature` shipped; `effect.reanimate` skipped because Fang-Druid Summoner puts the result INTO HAND, not onto the battlefield (not reanimate semantics).
- **Group 21 skipped** — `effect.bounce_artifact` PATTERN_BLINK_OWN deletion blocked: existing positive test row at `effect.bounce_artifact.test.ts:8` (`'exile target artifact, then return that artifact to the battlefield'`) would be silently flipped. Future PR needs to flip that row explicitly with Explosive Getaway as motivating negative.
- **Winter, Cursed Rider grants_ward** (from Group 11) — DEFERRED. Ward not in `GRANTABLE_KEYWORDS` at `effect.grants_keyword.ts:12-23`. Requires adding `ward` to the keyword list + a new `effect.grants_ward` tag.

---

## Shipped (rule changes landed; see commit message and v0.30.0 diff):
...
## The Ruinous Wrecking Crew  <!-- audited 2026-06-21, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## The Kingpin of Crime  <!-- audited 2026-06-21, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Okoye, Dora Milaje Leader  <!-- audited 2026-06-21, ruleVersion v0.48.0; resolved v0.49.0 -->
...
## A.I.M. Scientists  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Baron Helmut Zemo  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Beast, Erudite Aerialist  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0 -->
...
## Black Widow, Double Agent  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Captain America, Super-Soldier  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0 -->
...
## Captain America, Wings of Freedom  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Castle Doom  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->

> REJECTED: Villain-token producers already edge to Villain payoffs via effect.create_creature_token metadata.creatureTypes + the graph builder's tribe gate (graph.ts). cares_tribe stays narrowed per v0.49.0 Pattern 3.6.
...
## Claim the Kingdom  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Construct a Cosmic Cube  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->
...
## Doctor Doom  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->
...
## Hercules, Prince of Power  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## HYDRA Troopers  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->
...
## Invisible Woman, Sue Storm  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0/v0.50.0 -->
...
## Iron Fist, Living Weapon  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->

> REJECTED: quoted-grant ability internals are stripped by stripQuotedAbilities (normalize.ts) by design — hosts are tagged only from host-level text.
...
## King T'Challa // Black Panther, Hope Enduring  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->

> REJECTED: has_first_strike fires on double strike intentionally (superset design, metadata.doubleStrike — see effect.has_first_strike.ts).
...
## Leader, Super-Genius  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Luke Cage, Power Man  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->

> REJECTED: flat self-buffs are deliberately excluded from grants_stat_buff (only for-each-scaling self-buffs admitted); future has_stat_buff axis is the home for these.
...
## M.O.D.O.K.  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Mockingbird, Ace Agent  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->
...
## Okoye, Dora Milaje Leader  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0 -->
...
## Quicksilver, Brash Blur  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved/rejected v0.50.0 -->
...
## Rewrite History  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Robot Domination  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0/rejected v0.50.0 -->
...
## Super Intelligence  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0 -->
...
## Super-Skrull  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->
...
## The Kingpin of Crime  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## The Masters of Evil  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## The Mighty Thor, Jane Foster  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0/rejected v0.50.0 -->

> REJECTED: exile-then-immediate-return is blink — FLICKER_TAIL suppression working as designed; card carries effect.blink.
...
## The Sentry, Golden Guardian  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->

> REJECTED: opponent-actor token creation intentionally excluded (v0.14.9 controller-leak fix — the controller never gains the tokens).
...
## The Super Hero Civil War  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## The Thing, Ben Grimm  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0 -->
...
## The Vision  <!-- audited 2026-06-26, ruleVersion v0.48.0; rejected v0.50.0 -->
...
## Thunderbolts Conspiracy  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.50.0 -->
...
## Ultron, Artificial Malevolence  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0/v0.50.0 -->
...
## Worlds Within Worlds  <!-- audited 2026-06-26, ruleVersion v0.48.0; resolved v0.49.0 -->
